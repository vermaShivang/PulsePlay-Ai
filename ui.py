...
st.set_page_config(page_title="PulsePlay", layout="wide")
...
def create_user(name: str, email: str, password: str) -> Tuple[bool, str, Optional[int]]:
    """Insert a new user. Returns (ok, msg, user_id)."""
    try:
...
def verify_user(email: str, password: str) -> Tuple[bool, Optional[Dict]]:
    try:
        conn = get_db_connection()
        cur = conn.cursor()
...
def get_user_playlists(user_id: int) -> List[Dict]:
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT playlist_id, mood, playlist_name, created_at FROM playlist WHERE user_id = %s ORDER BY created_at DESC", (user_id,))
    rows = cur.fetchall()
    playlists = []
    for pid, mood, pname, created_at in rows:
        playlists.append({"playlist_id": pid, "mood": mood, "playlist_name": pname, "created_at": created_at})
    cur.close()
    conn.close()
    return playlists
...
def get_playlist_songs(playlist_id: int) -> List[Dict]:
    ...
    cur.close()
    conn.close()
    return [{"song_id": r[0], "title": r[1], "artist_name": r[2], "album_image_url": r[3]} for r in rows]


def create_playlist_for_user(user_id: int, playlist_name: str, mood: str, song_ids: List[int]) -> Tuple[bool, str, Optional[int]]:
    try:
        conn = get_db_connection()
        cur = conn.cursor()
            ...
            for p in playlists:
                ...
                with cols[0]:
                    if cover:
                        ...
                        st.empty()
                with cols[1]:
                    st.markdown(f"**{p['playlist_name']}** — mood: {p['mood']}")
                    st.write(f"Created: {p['created_at']}")
                    if songs:
                        for s in songs:
                            st.write(f"- {s['title']} — {s['artist_name']}")
                    else:
                        st.write("(no songs)")
                st.markdown("---")
        except Exception as e:
